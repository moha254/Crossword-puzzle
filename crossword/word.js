...
// After the player guesses the correct words
function playerCorrectGuess(words, grid) {
    // Check if all words have been correctly guessed
    var allWordsGuessed = true;
    ...
    // If all words have been correctly guessed, display the completed crossword
    if (allWordsGuessed) {
        // Generate HTML for clues and answers
        var legend = crossword.getLegend(grid);
        var cluesHtml = generateCluesHtml(legend);
        var answersHtml = CrosswordUtils.toHtml(grid, true); // Show answers

        // Display completed crossword grid and clues
        displayCompletedCrossword(answersHtml, cluesHtml);
    }
}

// Generate HTML for clues
function generateCluesHtml(legend) {
    var html = [];
    html.push("<div class='clues'>");
    html.push("<h2>Across</h2>");
    for (var i = 0; i < legend.across.length; i++) {
        var item = legend.across[i];
        html.push("<p><strong>" + item.position + "</strong>. " + item.clue + "</p>");
    }
    html.push("<h2>Down</h2>");
    for (var i = 0; i < legend.down.length; i++) {
        var item = legend.down[i];
        html.push("<p><strong>" + item.position + "</strong>. " + item.clue + "</p>");
    }
    html.push("</div>");
    return html.join("\n");
...
// Display completed crossword and clues
function displayCompletedCrossword(gridHtml, cluesHtml) {
    var crosswordContainer = document.getElementById("crossword-container");
    if (crosswordContainer) {
        crosswordContainer.innerHTML = gridHtml + cluesHtml;
    }
}
...
var CrosswordUtils = {
    ...
    toHtml : function(grid, show_answers){
        if(grid == null) return;
        var html = [];
        html.push("<table id='our_table' class='crossword'>");
        var label = 1;
        for(var r = 0; r < grid.length; r++){
            html.push("<tr>");
            for(var c = 0; c < grid[r].length; c++){
                ...
                if(cell == null){
                    ...
                    char = chars.substr( Math.floor(Math.random() * 27), 1);
                ...
                if(is_start_of_word) {
                    ...
                    html.push("<td class='" + css_class + "' title='" + r + ", " + c + "' style=\"zbackground-image:url('" + img_url + "')\">");
                    label++;			
                } else {
